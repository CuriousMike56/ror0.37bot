...
MSG2_CHAT  						= 1007                #!< chat line

MSG2_SERVER_SETTINGS			= 1008                #!< server send client the terrain name: server_info_t

MSG2_GAME_CMD  					= 1009                #!< send to client from server only

MSG2_USER_INFO  				= 1010                #!< user data that is sent from the server to the clients
MSG2_PRIVCHAT  					= 1011                #!< sent from client to server to send private chat messages
...
def processUserInfo(data):
	u = user_info_t()
	u.uniqueID, u.username, u.usertoken, u.serverpassword, u.language, u.clientname, u.clientversion, u.clientGUID, u.sessiontype, u.sessionoptions, u.authstatus, u.slotnum, u.colournum = struct.unpack('I20s40s40s10s10s25s40s10s128s3i', data)
...
def processServerInfo(data):
	...
	s.servername      = s.servername.strip('\0')
